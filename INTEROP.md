...
How **webrecon** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `webrecon` lives in the **Network / recon** cluster.
...
| from | relation | to |
|---|---|---|
| `webrecon` | JSON findings compose with | [`nmapparse`](https://github.com/cognis-digital/nmapparse), [`pcapview`](https://github.com/cognis-digital/pcapview), [`dnsrecon`](https://github.com/cognis-digital/dnsrecon), [`subhunt`](https://github.com/cognis-digital/subhunt) |
| `webrecon` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `webrecon` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `webrecon` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the network / recon cluster.** `webrecon` output feeds the next tool:
```bash
webrecon ... --format json > out.json          # this tool's findings (see `webrecon --help`)
...
webrecon ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
webrecon ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
webrecon ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `webrecon`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `webrecon` slots
